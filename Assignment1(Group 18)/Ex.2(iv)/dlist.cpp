...
int e1(){ //length function
    return 0;
}
DList<int> e2(){ //function function
    DList<int> list = DList<int>();
    return list;
}
DList<int> e3(){ //sublist function
    DList<int> list = DList<int>();
    return list;
}

int f1(int x){ //length function
    return 1;
}
DList<int> f2(int x){ //function function
    DList<int> list = DList<int>();
    x = x * x;  // let the function be F(x) = x ^ 2
    list.append(x);
    return list;
}
DList<int> f3(int x){ //sublist function
    DList<int> list = DList<int>();
    if(x > 0)  // let the condition be x is greater than 0
        list.append(x);
    return list;
}
int g1(int x, int y){ //length function
    return x + y;
}
DList<int> g2(DList<int> list1, DList<int> list2){ //function function
    list1.concat(&list2);
    return list1;
}
DList<int> g3(DList<int> list1, DList<int> list2){ //sublist function
    list1.concat(&list2);
    return list1;
}

template<class T> template<class Q> Q DList<T>::src(Q (*e)(), Q (*f)(T x), Q (*g)(Q l1, Q l2))
{
    node<T> *pt = dummy;
    Q ans = e();
    for(int i = 1; i <= numitems; i++){     // using a loop to implement structural recursion is easier
        pt = pt->getnext();
        ans = g(ans, f(pt->dataitem));
    }
    return ans;
}
