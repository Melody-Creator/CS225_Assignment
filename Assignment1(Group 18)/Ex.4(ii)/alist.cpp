...
template<class T> void AList<T>::insertionSort()
{
    for(int i = 2; i <= numitems; i++){
        int L = 0, R = i;
        T val = reprarray[i-1];
        while(L + 1 < R){   //use binary search to minimise the number of comparisons
            int mid = (L + R) >> 1;
            if(reprarray[mid-1] > val)  R = mid;
            else  L = mid;
        }
        for(int j = i; j > R; j--)  //move the list
            reprarray[j-1] = reprarray[j-2];
        reprarray[R-1] = val;   //insert the last element
    }
}
