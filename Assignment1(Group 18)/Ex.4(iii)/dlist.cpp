...
template<class T> void DList<T>::bubbleSort()
{
    for(int i = numitems-1; i >= 1; i--){  //the last position to compare with next
        node<T> *pt1 = dummy; 
        for(int j = 1; j <= i; j++){
            pt1 = pt1->getnext();
            node<T> *pt2 = pt1->getnext();
            if(pt1->dataitem >= pt2->dataitem){ //swap the adjacent elements
                T val = pt1->dataitem;
                pt1->dataitem = pt2->dataitem;
                pt2->dataitem= val;
            }
        } // after comparing and swapping, the i+1 th element is determined
    }
}
